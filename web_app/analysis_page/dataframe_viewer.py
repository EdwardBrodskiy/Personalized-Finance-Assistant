...
class DataFrameViewer(ctk.CTkFrame):
    def __init__(self, master, dataframe, **kwargs):
        ...
        self.grid_columnconfigure(list(range(4)), weight=1)
        ...
        self.up_key.grid(column=2, row=0, padx=5, pady=5)

        self.down_key = ctk.CTkButton(self, text='Down', command=self._go_down, state='disabled')
        self.down_key.grid(column=3, row=0, padx=5, pady=5)
    ...
    def set_button_states(self):
        ...
        self._set_button_state(self.down_key,
                               self.table is not None and self.table.row_of_interest < len(self.table.dataframe))
