...
class DataBase:
    @staticmethod
    def get_database():
        database = pd.read_csv('database/protected/all.csv', index_col='key')
    ...
    def add_to_database(new_items: pd.DataFrame):
        if len(new_items):
            ...
            database.to_csv('database/protected/previous_all.csv')

            # Merge and Save
            new_database = pd.concat([database, new_items], ignore_index=True)
            new_database.index.name = 'key'
            new_database.to_csv('database/protected/all.csv')
